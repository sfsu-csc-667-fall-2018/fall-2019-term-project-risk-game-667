    ...
    nextPhase(state) {
      ...
      if (startPhase !== state.phase && state.phase === Phase.DEPLOY) {
        ...
        if (state.turn > 0) {
          ...
          console.log(`El jugador ${i18n.es.players[state.player]} tiene ${newTroops} nuevas tropas`);
          console.log(` - ${troopsPerContinent} por continentes (${ownedContinents.join(", ")})`);
          console.log(` - ${troopsPerCountry} por países (${ownedCountries.join(", ")})`);
        }
      }
    }
  },
    ...
    buttonText() {
      ...
      if (state.phase === Phase.DEPLOY) {
        ...
        if (state.action.from === null) {
          ...
          return `Atacar con ${state.action.count} tropas`;
        }
        return 'Atacar';
      } else if (state.phase === Phase.MOVE) {
        if (state.action.from === null) {
          ...
          return `Mover a ${state.action.count} tropas`;
        }
        return 'Mover';
      }
    },
